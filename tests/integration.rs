...
fn test_setup_help_command() {
    Command::cargo_bin("workbloom")
        ...
        .stdout(predicate::str::contains("--no-mux"));
}
...
fn test_config_defaults() {
    ...
    assert!(config
        .claude_files
        .contains(&"settings.local.json".to_string()));
}
...
"#;
    fs::write(repo_path.join(".workbloom"), config_content).expect("Failed to write config");
