...
impl GitRepo {
    ...
    pub fn get_branch_last_commit_time(&self, branch_name: &str) -> Result<Option<SystemTime>> {
        validate_branch_name(branch_name)?;
        let output = Command::new("git")
        ...
        let timestamp = match stdout.trim().parse::<u64>() {
            Ok(ts) => ts,
...
mod tests {
    ...
    #[test]
    fn test_get_branch_last_commit_time() -> Result<()> {
        let (_temp_dir, repo) = setup_test_repo()?;

        let last_commit = repo
            .get_branch_last_commit_time("main")?
            .expect("expected commit time");
        let now = SystemTime::now();

        assert!(now.duration_since(last_commit).is_ok());
